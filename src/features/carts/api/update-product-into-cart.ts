...
async function updateProductFromCart({ cart }: UpdateProductIntoCartDTO): Promise<Cart> {
  return axios.put('/cart', JSON.stringify({ ...cart }))
}
...
export function useUpdateProductsFromCart() {
  ...
  return useMutation({
    onMutate: async (newCartProduct: any) => {
      ...
      queryClient.setQueryData(['carts'], [...(previousCartProducts || []), newCartProduct.data])
